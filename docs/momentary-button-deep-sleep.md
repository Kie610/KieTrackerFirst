...
- Run `INT1` to `D10` / GPIO9. It is not a wake source in this prototype and the
  firmware never reads it, but GPIO9 is an RTC GPIO, so leaving the wire in place
  keeps a later wake-on-motion path open without opening a finished assembly.
  `D9` / GPIO8 is the alternate. Neither is a strapping pin.
