...
| IMU INT | D10 / GPIO9 | Wired in the final build, but not used by LSM6DSV SoftFusion. RTC GPIO, so it stays available as a future EXT1 wake source |
...
`IMU INT` moved from GPIO4 to GPIO9 on 2026-08-04. GPIO8 and GPIO9 are both RTC
...
GPIO9 (`D10`) is in the right column near the GPIO7 (`D8`) button pad. Keeping
the button and INT1 on the same edge as `3V3` and `GND` removes wire crossings
from the perfboard layout. GPIO9 is the pin actually fitted and verified on
hardware on 2026-08-04; GPIO8 (`D9`) is the documented alternate and was the
proposal in `claude/momentary-deep-sleep-wiring-9f530a`, which was resolved in
favour of the built board. Neither is an ESP32-S3 strapping pin (those are
GPIO0, 3, 45, 46).
...
- common pins: SDA GPIO5, SCL GPIO6, IMU INT GPIO9, AUX INT GPIO2, battery ADC GPIO1
- power button: GPIO7, active low, 2000 ms hold
- battery monitoring: `BAT_EXTERNAL`, shield resistance 0, R1 100, R2 100, ADC multiplier 2, 15-sample median filter
